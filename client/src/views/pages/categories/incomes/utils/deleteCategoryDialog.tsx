...
const DeleteCategoryDialog = ({ open, onClose, incomeCate, onSubmit, loading }: any) => {
  return (
    <Dialog sx={{ '& .MuiDialog-paper': { overflow: 'visible' } }} open={open} onClose={onClose}>
      ...
      <DialogWithCustomCloseButton handleClose={onClose}>
        <Typography>Are you sure you want to delete {incomeCate.name}?</Typography>
...
export const ForceDeleteDialog = ({ open, onClose, incomeCate, onSubmit, loading }: any) => {
  return (
    <Dialog sx={{ '& .MuiDialog-paper': { overflow: 'visible' } }} open={open} onClose={onClose}>
      <Toaster position='top-right' toastOptions={{ className: 'react-hot-toast' }} />
      <DialogTitle>This category already has a note</DialogTitle>
      <DialogWithCustomCloseButton handleClose={onClose}>
        <Typography>Are you sure you want to force delete {incomeCate.name}?</Typography>
      </DialogWithCustomCloseButton>
      <DialogActions>
        <Button onClick={onClose}>Cancel</Button>
        <LoadingButton loading={loading} variant='contained' color='error' onClick={onSubmit}>
